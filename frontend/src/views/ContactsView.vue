<template>
  ...
  <div v-else class="smax-contacts-page">
    ...
    <!-- ════════ Tab: Active ↔ Thùng rác (Thùng rác = KH đã archive) ════════ -->
    <div v-if="canArchive" class="archive-tabs" role="tablist">
      <button
        type="button"
        class="archive-tab"
        :class="{ active: !viewArchived }"
        role="tab"
      ...
      >
        🗑 Thùng rác
        <span v-if="(total > 0) && viewArchived" class="archive-count">{{ total }}</span>
      </button>
    </div>

    <!-- ════════ Bulk action bar (hiện khi có chọn) ════════ -->
    <div v-if="canArchive && selectedCount > 0" class="bulk-bar">
      <span class="bulk-count">Đã chọn <b>{{ selectedCount }}</b> KH</span>
      <button type="button" class="bulk-clear" @click="clearSelection">Bỏ chọn</button>
      <div class="bulk-spacer" />
      <template v-if="!viewArchived">
        <button type="button" class="bulk-action bulk-action--danger" :disabled="bulkWorking" @click="onBulkArchive">
          🗑 Xóa (Thùng rác)
        </button>
      </template>
      <template v-else>
        <button type="button" class="bulk-action" :disabled="bulkWorking" @click="onBulkRestore">
          ♻ Khôi phục
        </button>
        <button v-if="isOwnerUser" type="button" class="bulk-action bulk-action--danger" :disabled="bulkWorking" @click="onBulkPurge">
          ✕ Xóa vĩnh viễn
        </button>
      </template>
    </div>
    ...
    <div class="toolbar toolbar-primary">
      ...
      <!-- Lọc theo Nick CRM (ZaloAccount) đang chăm KH — 2026-06-30 -->
      <select v-model="filters.zaloAccountId" @change="fetchContacts" title="Nick CRM (Zalo) đang chăm khách hàng">
        <option value="">Tất cả nick</option>
        <option v-for="n in allNicks" :key="n.id" :value="n.id">{{ n.displayName || n.phone || 'Nick' }}<template v-if="n.phone"> · {{ n.phone }}</template></option>
      </select>
    ...
    <!-- ════════ Pagination — top bar (2026-06-30) ════════
         Đặt trên đầu bảng KH để dễ nhảy trang khi list dài (anh chốt vị trí 3b).
         Gồm: Trước/Sau + số trang + chọn pageSize + nhảy đến trang đích. -->
    <div v-if="total > 0" class="cl-pager cl-pager--top">
      <button
        type="button" class="cl-pg-btn"
        :disabled="pagination.page <= 1"
        @click="changePage(pagination.page - 1)"
        title="Trang trước"
      >← Trước</button>
      <span class="cl-pg-info">Trang <b>{{ pagination.page }}</b> / {{ totalPages }}</span>
      <button
        type="button" class="cl-pg-btn"
        ...
        title="Trang sau"
      >Sau →</button>
      <span class="cl-pg-sep"></span>
      <label class="cl-pg-size">
        Hiển thị:
        <select v-model.number="pageSize" @change="onPageSizeChange" title="Số dòng trên 1 trang">
          <option v-for="n in PAGE_SIZE_OPTIONS" :key="n" :value="n">{{ n }} / trang</option>
        </select>
      </label>
      <span class="cl-pg-sep"></span>
      <label class="cl-pg-jump">
        Đến trang:
        <input
    ...
    <div class="scroll-wrap" :class="{ 'mode-shrunk': viewMode === 'm2' && selectedContact }">
      <table class="smax-table" :class="{ 'mode-shrunk': viewMode === 'm2' && selectedContact }">
        ...
        <colgroup>
          <col v-if="canArchive" style="width:34px">   <!-- 0 checkbox chọn (chỉ khi có quyền) -->
        ...
        <thead>
          <tr>
            <th v-if="canArchive" class="cell-check">
              <input
                type="checkbox"
                :checked="allSelected"
                @change="toggleSelectAll(($event.target as HTMLInputElement).checked)"
                title="Chọn tất cả KH đang hiển thị"
              />
            </th>
            <th></th>
            ...
            <th title="Nick CRM (Zalo) đang chăm KH — click avatar mở tab Bạn bè của nick">Nick CRM chăm</th>
            <th>KH nhắn cuối</th>
        ...
        <tbody>
          <template v-for="contact in contacts" :key="contact.id">
            ...
            >
              <!-- col0: checkbox chọn (chỉ khi có quyền archive) -->
              <td v-if="canArchive" class="cl-check-cell">
                <input
                  type="checkbox"
                  :checked="selectedIds.has(contact.id)"
                  @click.stop
                  @change="toggleSelectOne(contact.id, ($event.target as HTMLInputElement).checked)"
                />
              </td>
              <!-- col1: caret -->
              ...
              <!-- Nick CRM (ZaloAccount) đang chăm — avatar stack tối đa 3 + tên nick chính + +N (2026-06-30) -->
              <td class="cl-nicks-cell">
                <div v-if="linkedNicksOf(contact).length" class="cl-nicks" :title="nicksTooltip(contact)">
                  <span class="cl-nicks-avs">
                    <span
                      v-for="(nk, ni) in linkedNicksOf(contact).slice(0, 3)" :key="nk.id"
                      class="cl-nick-av"
                      :style="{ zIndex: 3 - ni }"
                      @click.stop="goToNickFriends(nk.id)"
                    >
                      <Avatar :src="nk.avatarUrl" :name="nk.displayName || nk.phone || '?'" :size="20" :gradient-seed="nk.id" />
                    </span>
                  </span>
                  <span class="cl-nick-name" :title="linkedNicksOf(contact)[0].displayName || ''" @click.stop="goToNickFriends(linkedNicksOf(contact)[0].id)">
                    {{ linkedNicksOf(contact)[0].displayName || linkedNicksOf(contact)[0].phone || 'Nick' }}
                  </span>
                  <span v-if="linkedNicksOf(contact).length > 1" class="cl-nick-more">+{{ linkedNicksOf(contact).length - 1 }}</span>
                </div>
                <span v-else class="empty">—</span>
              </td>
              <td>
              ...
              <td class="c-extra">
                <div class="cl-action">
                  ...
                  <!-- Nút archive/restore/purge tuỳ theo tab hiện tại — chỉ hiện khi có quyền -->
                  <button
                    v-if="canArchive && !viewArchived"
                    class="cl-btn cl-btn-danger"
                    @click.stop="onArchiveOne(contact)"
                    title="Chuyển vào Thùng rác"
                  >🗑</button>
                  <template v-else-if="canArchive && viewArchived">
                    <button class="cl-btn" @click.stop="onRestoreOne(contact)" title="Khôi phục về danh sách chính">♻</button>
                    <button
                      v-if="isOwnerUser"
                      class="cl-btn cl-btn-danger"
                      @click.stop="onPurgeOne(contact)"
                      title="Xóa vĩnh viễn (chỉ owner)"
                    >✕</button>
                  </template>
    ...
    <!-- Pagination — bottom bar (2026-06-30) -->
    <div v-if="total > 0" class="cl-pager cl-pager--bottom">
      <button
        type="button" class="cl-pg-btn"
        :disabled="pagination.page <= 1"
        @click="changePage(pagination.page - 1)"
...
const { contacts, total, loading, filters, pagination, fetchContacts, viewArchived, deleteContact, bulkArchiveContacts, restoreContact, bulkRestoreContacts, permanentDeleteContact, bulkPurgeContacts, setPageSize, goToPage } = useContacts();

// Cờ quyền: ai có thể archive/restore (grant contact.delete).
const canArchive = computed(() => authStore.canAccess('contact', 'delete'));
// Chỉ owner mới purge vĩnh viễn.
const isOwnerUser = computed(() => authStore.user?.role === 'owner');

// Selection cho bulk action. Set trong checkbox + header "select all".
...
function toggleSelectAll(checked: boolean) {
  selectedIds.value = checked ? new Set(contacts.value.map((c) => c.id)) : new Set();
}
function clearSelection() { selectedIds.value = new Set(); }

async function onArchiveOne(contact: Contact) {
  const ok = await confirm({
    title: 'Xóa khách hàng?',
    message: `Chuyển "${contact.fullName || contact.crmName || contact.id}" vào Thùng rác? Có thể khôi phục trong tab Thùng rác.`,
  ...
  if (success) toast.success('Đã chuyển vào thùng rác');
}
...
async function onBulkArchive() {
  ...
  const ok = await confirm({
    title: `Xóa ${ids.length} khách hàng?`,
    message: `Chuyển ${ids.length} KH vào Thùng rác? Có thể khôi phục trong tab Thùng rác.`,
    confirmText: 'Chuyển vào thùng rác',
    cancelText: 'Hủy',
    tone: 'danger',
  });
  ...
  toast.success(`Đã chuyển ${n} KH vào thùng rác`);
}

async function onRestoreOne(contact: Contact) {
  const ok = await confirm({
    title: 'Khôi phục khách hàng?',
    message: `Khôi phục "${contact.fullName || contact.crmName || contact.id}" về danh sách chính?`,
    confirmText: 'Khôi phục',
    cancelText: 'Hủy',
  });
  if (!ok) return;
  const success = await restoreContact(contact.id);
  if (success) toast.success('Đã khôi phục');
}
...
async function onBulkRestore() {
  ...
  clearSelection();
  bulkWorking.value = false;
  toast.success(`Đã khôi phục ${n} KH`);
}

async function onPurgeOne(contact: Contact) {
  if (!isOwnerUser.value) return;
  const ok = await confirm({
    title: 'Xóa vĩnh viễn?',
    message: `Xóa VĨNH VIỄN "${contact.fullName || contact.crmName || contact.id}"? Hành động không thể hoàn tác — toàn bộ hồ sơ, tin nhắn liên quan sẽ bị xóa.`,
    confirmText: 'Xóa vĩnh viễn',
    cancelText: 'Hủy',
    tone: 'danger',
  });
  if (!ok) return;
  const success = await permanentDeleteContact(contact.id);
  if (success) toast.success('Đã xóa vĩnh viễn');
}
...
async function onBulkPurge() {
  ...
  if (!isOwnerUser.value) return;
  const ok = await confirm({
    title: `Xóa vĩnh viễn ${ids.length} KH?`,
    message: `Hành động KHÔNG thể hoàn tác. Toàn bộ hồ sơ + tin nhắn liên quan sẽ bị xóa khỏi hệ thống.`,
    confirmText: 'Xóa vĩnh viễn tất cả',
    cancelText: 'Hủy',
    tone: 'danger',
  });
  if (!ok) return;
  ...
  clearSelection();
  bulkWorking.value = false;
  toast.success(`Đã xóa vĩnh viễn ${n} KH`);
}
...
// Nick CRM (ZaloAccount) cho dropdown filter "Nick CRM chăm" — 2026-06-30.
// Chỉ list nick còn dùng được (status connected + chưa archive).
interface NickLite { id: string; displayName: string | null; phone: string | null }
const allNicks = ref<NickLite[]>([]);
async function loadNicks() {
  if (allNicks.value.length > 0) return;
  try {
    const res = await api.get<Array<{ id: string; displayName: string | null; phone: string | null; status?: string; archivedAt?: string | null }>>('/zalo-accounts');
    allNicks.value = (res.data || [])
      .filter((a) => !a.archivedAt)
      .map((a) => ({ id: a.id, displayName: a.displayName, phone: a.phone }));
  } catch {
    allNicks.value = [];
  }
}

// Tooltip cột Nick chăm: list đầy đủ "tên nick · SĐT · sale owner".
function nicksTooltip(contact: Contact): string {
  return linkedNicksOf(contact)
    .map((n) => {
      const parts = [n.displayName || n.phone || 'Nick'];
      if (n.phone && n.displayName) parts.push(n.phone);
      if (n.ownerName) parts.push('(' + n.ownerName + ')');
      return parts.join(' · ');
    })
    .join('\n');
}

// Click avatar/tên nick → mở tab Bạn bè lọc theo nick đó.
function goToNickFriends(zaloAccountId: string) {
  router.push({ path: '/friends', query: { zaloAccountId } });
}
...
// pageSize select bind trực tiếp vào pagination.limit (set bởi composable để
// persist localStorage); jumpPage là input tạm — commit bằng Enter hoặc nút "Đến".
const pageSize = ref(pagination.limit);
const jumpPage = ref(pagination.page);

// Scroll bảng lên đầu khi đổi trang/pageSize — UX tốt hơn khi bảng dài.
function scrollTableToTop() {
  const el = document.querySelector('.smax-contacts-page .smax-table');
  el?.scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function changePage(p: number) {
  // Clear selection: IDs của trang cũ không còn ý nghĩa ở trang mới.
...
function onPageSizeChange() {
  if (selectedIds.value.size > 0) clearSelection();
  jumpPage.value = 1;
  // pageSize ref đã được <select v-model.number> cập nhật; truyền xuống composable
  // để persist localStorage + reload.
  void setPageSize(pageSize.value);
  scrollTableToTop();
}

function onJumpToPage() {
  // goToPage sẽ clamp trong composable. Clear selection + scroll top.
  if (selectedIds.value.size > 0) clearSelection();
  void goToPage(jumpPage.value, totalPages.value);
  // Sync jumpPage về giá trị thực tế sau clamp (sẽ update qua watch).
  scrollTableToTop();
}

// Sync jumpPage khi page đổi do action khác (changePage, đổi tab, sort...).
watch(
  () => pagination.page,
  (newPage) => { jumpPage.value = newPage; },
);
...
async function fetchFriendships(contact: Contact) {
  ...
  try {
    ...
    // Override type `friends`: endpoint detail trả ApiFriendship[] (đầy đủ per-pair),
    // khác type list Contact.friends (lite, chỉ zaloAccount cho cột Nick chăm).
    const res = await api.get<Omit<Contact, 'friends'> & { friends?: ApiFriendship[] }>(`/contacts/${contact.id}`);
...
/* ── Pagination bar (2026-06-30) — top + bottom ──────────────────────
   Layout flex 1 dòng: Trước/Sau + số trang + pageSize select + jump input.
   Tone Vuetify grey-700, button dùng .cl-pg-btn để bám style toolbar.
   Top bar dính ngay sau stats; bottom bar ở cuối trang. */
.cl-pager {
...
.cl-pg-btn:hover:not(:disabled) {
  background: var(--smax-grey-100, #f3f4f6);
  border-color: var(--smax-grey-400, #9ca3af);
}
.cl-pg-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.cl-pg-info { font-variant-numeric: tabular-nums; min-width: 100px; text-align: center; }
.cl-pg-info b { color: var(--smax-grey-900, #111827); }
.cl-pg-sep {
  display: inline-block;
  width: 1px; height: 18px;
  background: var(--smax-grey-300, #d1d5db);
  margin: 0 4px;
}
.cl-pg-size,
.cl-pg-jump {
...
.cl-pg-size select { cursor: pointer; }
.cl-pg-size select:focus,
.cl-pg-jump input:focus {
  outline: 2px solid var(--brand, #1786be);
  outline-offset: -1px;
  border-color: transparent;
}
.cl-pg-jump input { width: 60px; text-align: center; }
.cl-pg-jump input:disabled { background: var(--smax-grey-100, #f3f4f6); cursor: not-allowed; }
...
/* ── Cột Nick CRM chăm (2026-06-30) ── avatar stack + tên nick chính + +N */
.cl-nicks-cell { overflow: hidden; }
.cl-nicks { display: flex; align-items: center; gap: 4px; min-width: 0; }
.cl-nicks-avs { display: inline-flex; flex-shrink: 0; }
.cl-nick-av {
  display: inline-flex; border-radius: 50%; cursor: pointer;
  margin-left: -6px; border: 1.5px solid #fff; position: relative;
  transition: transform 0.1s;
}
.cl-nick-av:first-child { margin-left: 0; }
.cl-nick-av:hover { transform: translateY(-1px); z-index: 10 !important; }
.cl-nick-name {
  font-size: 11px; color: var(--smax-grey-800, #1f2937);
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  cursor: pointer; min-width: 0; flex: 1;
}
.cl-nick-name:hover { color: var(--brand, #1786be); text-decoration: underline; }
.cl-nick-more {
...
/* ── Tab Active ↔ Thùng rác (Soft delete 2026-06-30) ───────────────────── */
.archive-tabs {
  display: flex; gap: 4px;
  padding: 6px 14px 0;
  border-bottom: 1px solid var(--at-hairline, #e2e8f0);
}
.archive-tab {
  background: transparent;
  ...
  color: var(--at-hint, #6b7488);
  cursor: pointer;
  display: inline-flex; align-items: center; gap: 6px;
}
.archive-tab:hover { background: var(--at-surface-soft, #f8fafc); color: var(--at-ink, #141a24); }
.archive-tab.active {
  background: #fff;
  color: var(--at-ink, #141a24);
  border-color: var(--at-hairline, #e2e8f0);
...
.archive-count {
  ...
  border-radius: 999px;
  background: var(--at-surface-soft, #f1f5f9);
  color: var(--at-hint, #475066);
  font-size: 11px; font-weight: 700;
  text-align: center;
}

/* ── Bulk action bar (2026-06-30) ────────────────────────────────────────── */
.bulk-bar {
...
.bulk-count { color: #1e40af; font-weight: 600; }
.bulk-spacer { flex: 1; }
.bulk-clear {
...
.bulk-action:hover:not(:disabled) { background: #dbeafe; }
.bulk-action:disabled { opacity: 0.55; cursor: not-allowed; }
.bulk-action--danger {
  border-color: #fca5a5;
  background: #fff1f2;
  color: #991b1b;
}
.bulk-action--danger:hover:not(:disabled) { background: #fee2e2; }

/* ── Checkbox cell dòng KH Cha ───────────────────────────────────────────── */
.cell-check { text-align: center; padding: 0 4px; }
.cl-check-cell { text-align: center; padding: 0 4px; }
.cell-check input[type="checkbox"],
.cl-check-cell input[type="checkbox"] {
  cursor: pointer;
  width: 15px; height: 15px;
  accent-color: var(--at-action, #1786be);
}

/* ── Nút danger trong cụm Action dòng KH Cha ────────────────────────────── */
.cl-btn-danger {
  color: #b91c1c;
  background: #fef2f2;
  border-color: #fecaca;
}
.cl-btn-danger:hover { background: #fee2e2; border-color: #fca5a5; }
