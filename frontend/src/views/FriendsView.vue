      ...
      <main class="main">
        ...
        <div class="pag">
          ...
          <input
            ...
            @keyup.enter="onJump"
          />
          <button class="pag-jump-btn" :disabled="!jumpInput" @click="onJump">Đi</button>
          <span class="pag-sep">·</span>
          <span>Trang:</span>
...
const LS_KEY_PAGE_SIZE = 'friendsview.pageSize.v1';
function loadPageSize(): number {
  try {
    const raw = Number(localStorage.getItem(LS_KEY_PAGE_SIZE));
    if (PAGE_SIZE_OPTIONS.includes(raw as typeof PAGE_SIZE_OPTIONS[number])) return raw;
  } catch { /* ignore */ }
  return 25;
}
...
// Đổi số dòng/trang. Reset về trang 1 để không lệch vị trí. Persist localStorage.
function setPageSize(n: number) {
  const safe = PAGE_SIZE_OPTIONS.includes(n as typeof PAGE_SIZE_OPTIONS[number]) ? n : 25;
...
// Nhảy đến trang cụ thể — clamp vào [1, totalPages], bỏ qua nếu trùng trang hiện tại.
const jumpInput = ref('');
function onJump() {
  const n = Math.floor(Number(jumpInput.value));
  if (!Number.isFinite(n) || n < 1) { jumpInput.value = ''; return; }
...
.pag-select:hover { background: var(--surface-3); }
.pag-jump {
...
.pag-jump:focus { outline: none; border-color: var(--brand); box-shadow: 0 0 0 3px var(--brand-soft); }
.pag-jump-btn {
  padding: 5px 11px;
  border: 1px solid var(--line); background: var(--surface);
  border-radius: var(--r-sm); cursor: pointer; font-size: 12.5px; font-weight: 600;
...
.pag-jump-btn:hover:not(:disabled) { background: var(--surface-3); }
.pag-jump-btn:disabled { opacity: .4; cursor: not-allowed; }
