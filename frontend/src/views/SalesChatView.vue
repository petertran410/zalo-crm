<template>
  ...
  >
    ...
    <div
      v-if="currentRole === 'sales' && showContactPanel && selectedConv"
      class="sl-resizer"
...
const currentRole = ref<string>('sales');
/** true = ẩn thanh tìm kiếm + bộ lọc, chỉ hiện danh sách hội thoại */
const filterCollapsed = ref(false);
// Sales bị khoá cứng — không cho switch sang workspace khác qua role selector.
// Watcher chỉ giữ lại nhánh 'cs' phòng trường hợp component bị tái sử dụng,
// nhánh 'manager' bị xoá vì SalesChatView không bao giờ dùng DefaultLayout.
watch(currentRole, (val) => {
  if (val === 'cs') {
    router.push({ name: 'CsChat', params: route.params });
  }
  // 'sales' = giữ nguyên; 'manager' = không làm gì (sales không được switch)
});
...
function onConversationDeleted(id: string) {
  ...
  if (selectedConvId.value === id) {
    // FIX: navigate về SalesChat (không phải Chat/DefaultLayout)
    router.push({ name: 'SalesChat' }).catch(() => {});
  }
  fetchConversations({ bypassCache: true });
  void refreshPriorityUnread();
}
...
async function onComposeOpened(conversationId: string) {
  ...
  // FIX: giữ trong SalesChat để không thoát khỏi Sales Workspace
  router.push({ name: 'SalesChat', params: { convId: conversationId } });
}
...
/** Khi user click 1 conv → push URL /sales-chat/:id (watcher bên dưới sẽ trigger selectConversation) */
function onSelectConv(convId: string) {
  ...
  // FIX ROOT CAUSE: 'Chat' → 'SalesChat' để giữ Sales Layout khi chọn conversation.
  // Route 'Chat' (/chat) dùng DefaultLayout, 'SalesChat' (/sales-chat) dùng SalesLayout.
  router.push({ name: 'SalesChat', params: { convId } });
}
...
// Watch query.contactId — khi nav từ Contacts/Friends qua /sales-chat?contactId=xxx
// Resolve sang convId qua conversations list, rồi redirect /sales-chat/:convId.
watch(
  ...
  ([contactId, convs]) => {
    ...
    if (match) {
      // FIX: giữ trong SalesChat khi resolve từ contactId query
      router.replace({ name: 'SalesChat', params: { convId: match.id } });
    }
  },
