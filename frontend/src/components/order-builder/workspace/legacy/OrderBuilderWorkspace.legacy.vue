                ...
                >
                  ...
                  <template #customer>
                    <Section1Customer
                      ...
                      :package-length="draft?.packageLength"
                      :package-width="draft?.packageWidth"
                      :package-height="draft?.packageHeight"
                      :package-weight="draft?.packageWeight"
                      @select-branch="handleSelectBranch"
                      @select-payment="handleSelectPayment"
                      @update-delivery-address="handleUpdateDeliveryAddress"
                      @update-package-metrics="handleUpdatePackageMetrics"
                    />
                  </template>
                  ...
                  <template #products>
                    <Section2Products
                      ...
                      :order-discount-value="draft?.orderDiscountValue || 0"
                      :order-discount-type="draft?.orderDiscountType || 'amount'"
                      :grand-total="grandTotal"
                      :selected-price-book-id="draft?.priceBookId"
                      :applied-promo-ids="draft?.appliedPromoIds ?? []"
                      :branches="branches"
                      :selected-branch-id="draft?.branchId ?? null"
                      :pos-customer-id="draft?.posCustomerId ?? null"
                      @add-product="handleAddProduct"
                      @update-cart-item="handleUpdateCartItem"
                      @update-quantity="handleUpdateQuantity"
                      @update-product-note="handleUpdateProductNote"
                      @remove-product="handleRemoveProduct"
                      @update-product-discount="handleUpdateProductDiscount"
                      @update-order-discount="handleUpdateOrderDiscount"
                      @update-order-discount-type="handleUpdateOrderDiscountType"
                  ...
                  <!-- Section 3: Xem lại & Chốt đơn -->
                  <template #review-checkout>
                    <Section3ReviewCheckout
                      :customer="customerInfo"
                      :cart-items="cartItems"
...
import { User, Sparkles, Zap } from 'lucide-vue-next';
import { api } from '@/api';
...
import Section3ReviewCheckout from './Section3ReviewCheckout.vue';
...
import type {
  POSProduct, POSBranch, CustomerInfo, PromotionProgram,
} from '../types';
...
const lockedSections = computed<string[]>(() => {
  ...
  if (realItems.length === 0) locked.push('review-checkout');
  return locked;
});
...
const accordionSections = computed<AccordionSection[]>(() => [
  {
    ...
    icon: User,
    summary: customerInfo.value.name ? `${customerInfo.value.name}${selectedBranch.value ? ` · ${selectedBranch.value.name}` : ''}` : undefined,
    nextLabel: 'Tiếp tục chọn sản phẩm',
  },
  {
    id: 'products',
  ...
  },
  {
    ...
    icon: Sparkles,
    lockReason: 'Thêm sản phẩm trước khi chốt đơn',
    showNext: false,
  },
]);
...
function handleNextSection() {
  const sectionOrder = ['customer', 'products', 'review-checkout'];
...
function handleUpdateBillNote(note: string) {
  draftStore.updateDraft(props.draftId, { billNote: note });
}
function handleUpdateShippingNote(note: string) {
  draftStore.updateDraft(props.draftId, { shippingNote: note, description: note });
}
...
// ─── Cart Operations ──────────────────────────────────────────────────
function handleAddProduct(product: POSProduct, opts?: { quantity?: number; discount?: number; note?: string; conditionType?: string; discountType?: string; discountValue?: number }) {
  if (!draft.value) return;
  const items = [...cartItems.value];
  ...
  const discountType = opts?.discountType as 'amount' | 'percent' | undefined;
  const discountValue = opts?.discountValue;

  const existingIdx = items.findIndex(c => c.product.id === product.id && !c.isGift && (c.note || '') === note && (c.discount || 0) === discount && c.conditionType === conditionType && c.discountType === discountType && c.discountValue === discountValue);
  if (existingIdx !== -1) {
...
function handleUpdateCartItem(productId: number, opts: { quantity: number; discount: number; note: string; conditionType?: string; discountType?: string; discountValue?: number }) {
  if (!draft.value) return;
  const items = [...cartItems.value];
  const idx = items.findIndex(c => c.product.id === productId && !c.isGift);
  if (idx !== -1) {
    items[idx] = {
      ...items[idx],
      ...
      conditionType: opts.conditionType as any,
      discountType: opts.discountType as any,
      discountValue: opts.discountValue,
    };
    draftStore.updateDraft(props.draftId, { cartItems: items });
    showToast(`Đã cập nhật ${items[idx].product.name}.`);
  }
}

function handleUpdateProductNote(index: number, note: string) {
  const items = [...cartItems.value];
  if (items[index]) {
    items[index].note = note || undefined;
    draftStore.updateDraft(props.draftId, { cartItems: items });
  }
}
...
.ob-rail-step:active:not(.ob-rail-step--locked) {
