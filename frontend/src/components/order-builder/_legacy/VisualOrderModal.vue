        ...
        <div class="ob-modal-frame">
          ...
          <div class="ob-modal__header">
            ...
            <div class="ob-modal__header-right">
              <button class="ob-modal__reset-btn" title="Đặt lại sơ đồ về mặc định" @click="handleReset">
                <Zap :size="14" class="ob-text-amber" />
                <span>Đặt lại sơ đồ</span>
              </button>
              <div class="ob-win-controls">
                <button class="ob-win-btn ob-win-btn--minimize" title="Thu nhỏ" @click="isMinimized = true">
                  <Minus :size="15" />
                </button>
          ...
          <div class="ob-modal__workspace">
            ...
            <SidebarPalette
              ...
              :applied-promo-ids="activeDraft?.appliedPromoIds ?? []"
              ...
              @promo-became-eligible="handlePromoBecameEligible"
            />
...
import { Zap, ShoppingBag, Maximize2, Minimize2, AlertTriangle, Minus, X } from 'lucide-vue-next';
import { api } from '@/api';
...
import type {
  POSProduct, POSBranch, CustomerInfo, CartItem, DraftOrder, PromotionProgram,
} from './types';
...
function handleClearOrder() {
  ...
  activeDraft.value.appliedPromoIds = [];
...
function handleApplyPromotion(promo: PromotionProgram) {
  ...
  // Tránh áp dụng 2 lần
  if (draft.appliedPromoIds?.includes(promo.id)) return;
  ...
  if (promo.reward.type === 'free_product' && promo.reward.giftProduct) {
    ...
    // Kiểm tra xem quà này đã có trong giỏ chưa
    const existing = draft.cartItems.find(c => c.product.id === gift.id && c.isGift);
    if (existing) {
      ...
      draft.cartItems.push({
        product: gift,
        quantity: qty,
        discount: gift.basePrice * qty,  // price = 0 (100% discount)
        isGift: true,
        promoId: promo.id,
      });
    }
    showToast(`🎁 Đã thêm ${qty} ${gift.unit || 'sản phẩm'} "${gift.name}" miễn phí!`);

  } else if (promo.reward.type === 'order_discount' && promo.reward.discountAmount) {
    // Tăng chiết khấu tổng đơn
    draft.orderDiscount = (draft.orderDiscount || 0) + promo.reward.discountAmount;
    showToast(`💰 Đã giảm thêm ${formatVND(promo.reward.discountAmount)} vào đơn hàng!`);
  }

  if (!draft.appliedPromoIds) draft.appliedPromoIds = [];
  draft.appliedPromoIds.push(promo.id);
}
...
function handleRemovePromotion(promoId: string) {
  ...
  // Xóa quà tặng khỏi giỏ
  const giftIdx = draft.cartItems.findIndex(c => c.isGift && c.promoId === promoId);
  if (giftIdx !== -1) draft.cartItems.splice(giftIdx, 1);

  // Hoàn lại discount nếu là order_discount
  const promo = MOCK_PROMOTIONS.find(p => p.id === promoId);
  if (promo?.reward.type === 'order_discount' && promo.reward.discountAmount) {
    draft.orderDiscount = Math.max(0, (draft.orderDiscount || 0) - promo.reward.discountAmount);
  }

  // Xóa khỏi danh sách applied
  if (draft.appliedPromoIds) {
    const idx = draft.appliedPromoIds.indexOf(promoId);
    if (idx !== -1) draft.appliedPromoIds.splice(idx, 1);
  }
  showToast('Đã hủy khuyến mãi.');
}

/** Khi một promo vừa đủ điều kiện: hiển thị toast xanh gợi ý */
function handlePromoBecameEligible(promo: PromotionProgram) {
  showToast(`🎉 Đủ điều kiện "${promo.tag}" — ${promo.rewardText}. Chuyển tab Khuyến mãi để áp dụng!`);
}

/** Tự động hủy các khuyến mãi đã áp dụng nếu giỏ hàng không còn thỏa mãn điều kiện */
function checkAndAutoRemoveIneligiblePromotions() {
  if (!activeDraft.value || !activeDraft.value.appliedPromoIds || activeDraft.value.appliedPromoIds.length === 0) return;
  const draft = activeDraft.value;
  const realItems = draft.cartItems.filter(c => !c.isGift);
  const pbId = draft.priceBookId || 'standard';

  // Tính tổng tiền sản phẩm thật trước KM
  const currentTotal = realItems.reduce((sum, item) => {
    const unitPrice = getEffectiveProductPrice(item.product.basePrice, pbId);
  ...
  for (const promoId of appliedIds) {
    const promo = MOCK_PROMOTIONS.find(p => p.id === promoId);
    if (promo) {
      const isStillEligible = evaluatePromoCondition(promo.condition, draft.cartItems, currentTotal);
      if (!isStillEligible) {
        handleRemovePromotion(promoId);
        showToast(`⚠️ Không còn đủ điều kiện "${promo.tag}" — Đã hủy khuyến mãi.`);
      }
    }
  }
}

// Watch giỏ hàng và bảng giá: tự động hủy khuyến mãi nếu không còn đủ ĐK
watch(
  () => [activeDraft.value?.cartItems, activeDraft.value?.priceBookId],
  () => {
    checkAndAutoRemoveIneligiblePromotions();
  },
  { deep: true },
);
...
/* ═══ Wrapper: modal frame + mini chat side-by-side ═══ */
.ob-modal-wrapper {
  display: flex;
  flex-direction: row;
  align-items: stretch;
  gap: 10px;
  width: 99.5vw;
  max-width: clamp(1400px, 96vw, 2400px);
  height: 95vh;
...
.ob-win-btn--minimize:hover {
  background: #f1f5f9;
  color: #0068FF;
  border-color: #cbd5e1;
}
.ob-win-btn--close:hover {
  background: #ef4444;
  color: #ffffff;
  border-color: #ef4444;
}
