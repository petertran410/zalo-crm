<template>
  <div class="cfb" :class="{ 'cfb--collapsed': collapsed }">
    <!-- ══ COLLAPSIBLE FILTER AREA ══ -->
    <div class="cfb-collapsible" :class="{ 'cfb-collapsible--open': !collapsed }">
    ...
    <!-- ② SALES FILTER BAR — dropdown buttons (chỉ hiện khi role = sales) -->
    <div v-if="currentRole === 'sales'" class="cfb-sales-bar" ref="salesBarRef">
      <div class="cfb-sales-btns" style="padding: 5px 8px;">
        <!-- Tag -->
        <button
          type="button"
          class="sfb-btn"
          :class="{ active: tagActiveCount > 0, open: openDropdown === 'tag' }"
          @click.stop="toggleDropdown('tag')"
          title="Lọc theo Tag"
        >
          <TagIcon :size="12" :stroke-width="2" />
          <span>Tag</span>
          <span v-if="tagActiveCount > 0" class="sfb-badge">{{ tagActiveCount }}</span>
          <ChevronDownIcon :size="10" :stroke-width="2.5" class="sfb-caret" :class="{ open: openDropdown === 'tag' }" />
        </button>
        ...
        <button
          type="button"
          class="sfb-btn"
          :class="{ active: msgActiveCount > 0, open: openDropdown === 'message' }"
          @click.stop="toggleDropdown('message')"
          title="Lọc theo trạng thái tin nhắn"
        >
          <InboxIcon :size="12" :stroke-width="2" />
          <span>Tin nhắn</span>
          <span v-if="msgActiveCount > 0" class="sfb-badge">{{ msgActiveCount }}</span>
          <ChevronDownIcon :size="10" :stroke-width="2.5" class="sfb-caret" :class="{ open: openDropdown === 'message' }" />
        </button>
        ...
        <button
          type="button"
          class="sfb-btn"
          :class="{ active: eventActiveCount > 0, open: openDropdown === 'event' }"
          @click.stop="toggleDropdown('event')"
          title="Lọc theo sự kiện & lịch hẹn"
        >
          <CalendarClockIcon :size="12" :stroke-width="2" />
          <span>Lịch hẹn</span>
          <span v-if="eventActiveCount > 0" class="sfb-badge">{{ eventActiveCount }}</span>
          <ChevronDownIcon :size="10" :stroke-width="2.5" class="sfb-caret" :class="{ open: openDropdown === 'event' }" />
        </button>
        ...
        <button
          type="button"
          class="sfb-btn"
          :class="{ active: saleActiveCount > 0, open: openDropdown === 'sale' }"
          @click.stop="toggleDropdown('sale')"
          title="Lọc theo Sale phụ trách"
        >
          <UserRoundCogIcon :size="12" :stroke-width="2" />
          <span>Phụ trách</span>
          <span v-if="saleActiveCount > 0" class="sfb-badge">{{ saleActiveCount }}</span>
          <ChevronDownIcon :size="10" :stroke-width="2.5" class="sfb-caret" :class="{ open: openDropdown === 'sale' }" />
        </button>

        <!-- Divider + Chip tích hợp -->
        <template v-if="salesHasActiveFilter">
          <div class="sfb-divider" aria-hidden="true"></div>
          <button
            v-for="chip in salesActiveChips"
            :key="chip.key"
            type="button"
            class="sfb-chip-inline"
            @click.stop="chip.remove()"
            :title="'Bỏ lọc: ' + chip.label"
          >{{ chip.label }}<XIcon :size="10" :stroke-width="2.5" /></button>
        </template>

        <!-- Nút xoá tất cả filter -->
        <button
          v-if="salesHasActiveFilter"
          type="button"
          class="sfb-clear"
          title="Xoá tất cả bộ lọc"
          @click.stop="clearSalesFilters"
        >
      ...
      <!-- ══ TAG DROPDOWN ══ -->
      <div v-if="openDropdown === 'tag'" class="sfb-panel" @click.stop>
        <div class="sfb-panel-header">
          <span class="sfb-panel-title"><TagIcon :size="13" :stroke-width="2" /> Tag</span>
          <button class="sfb-panel-close" @click="openDropdown = null"><XIcon :size="14" :stroke-width="2" /></button>
        </div>
        <div class="sfb-panel-body">
          <!-- Tag search -->
          <div class="sfb-search-row">
            <SearchIcon :size="13" :stroke-width="2" />
            <input v-model="tagSearch" type="text" placeholder="Tìm tag..." class="sfb-search-input" autocomplete="off" />
            <button v-if="tagSearch" class="sfb-search-clear" @click="tagSearch = ''"><XIcon :size="12" :stroke-width="2" /></button>
          </div>

          <!-- CRM Tags -->
          <div v-if="filteredCrmTags.length > 0" class="sfb-sub-label">Tag CRM</div>
          <div v-if="filteredCrmTags.length > 0" class="sfb-chip-wrap">
            <button
            ...
            >
              <span v-if="tag.emoji" class="sfb-chip-emoji">{{ tag.emoji }}</span>
              {{ tag.name }}
            </button>
          </div>

          <!-- Zalo Tags -->
          <div v-if="filteredZaloTags.length > 0" class="sfb-sub-label">Tag Zalo</div>
          <div v-if="filteredZaloTags.length > 0" class="sfb-chip-wrap">
            <button
              v-for="tag in filteredZaloTags"
              :key="'zalo-' + tag.id"
              type="button"
              class="sfb-chip sfb-chip--zalo"
              :class="{ selected: filters.state.tagsZalo.includes(tag.name) }"
              @click="toggleZaloTag(tag.name)"
            >
              {{ tag.name }}
            </button>
          </div>
          ...
          <!-- Auto-tags -->
          <div class="sfb-sub-label">Auto-tag</div>
          <div class="sfb-chip-wrap">
            <button
          ...
          <div v-if="tagSearch && filteredCrmTags.length === 0 && filteredZaloTags.length === 0" class="sfb-empty">
      ...
      <!-- ══ TIN NHẮN DROPDOWN ══ -->
      <div v-if="openDropdown === 'message'" class="sfb-panel" @click.stop>
        <div class="sfb-panel-header">
          <span class="sfb-panel-title"><InboxIcon :size="13" :stroke-width="2" /> Tin nhắn</span>
          <button class="sfb-panel-close" @click="openDropdown = null"><XIcon :size="14" :stroke-width="2" /></button>
        </div>
        <div class="sfb-panel-body">
          <div class="sfb-sub-label">Người trả lời cuối</div>
          <div
          ...
          >
            <div class="sfb-row-left">
              <span class="sfb-row-icon"><component :is="m.icon" :size="13" :stroke-width="2" /></span>
              <span class="sfb-row-lbl">{{ m.label }}</span>
            </div>
            <span v-if="eventCounts[m.countKey] > 0" class="sfb-row-count" :class="{ red: m.key === 'bot_no_sale' }">
              {{ eventCounts[m.countKey] }}
            </span>
          </div>

          <div class="sfb-sub-label" style="margin-top:10px">Cờ tương tác</div>
          <div
            class="sfb-row"
            :class="{ checked: filters.state.customerWaitingReply }"
            title="KH nhắn cuối, chưa sale reply"
            @click="filters.state.customerWaitingReply = !filters.state.customerWaitingReply"
          >
            <div class="sfb-row-left"><span class="sfb-check-box"></span><span class="sfb-row-lbl">KH chờ sale reply</span></div>
          </div>
          <div
            class="sfb-row"
            :class="{ checked: filters.state.saleWaitingReply }"
            title="Sale nhắn cuối, chờ KH reply"
            @click="filters.state.saleWaitingReply = !filters.state.saleWaitingReply"
          >
            <div class="sfb-row-left"><span class="sfb-check-box"></span><span class="sfb-row-lbl">Sale chờ KH reply</span></div>
          </div>
        </div>
      </div>

      <!-- ══ LỊCH HẸN DROPDOWN ══ -->
      <div v-if="openDropdown === 'event'" class="sfb-panel" @click.stop>
        <div class="sfb-panel-header">
          <span class="sfb-panel-title"><CalendarClockIcon :size="13" :stroke-width="2" /> Lịch hẹn</span>
          <button class="sfb-panel-close" @click="openDropdown = null"><XIcon :size="14" :stroke-width="2" /></button>
        </div>
        <div class="sfb-panel-body">
          <div
            class="sfb-row"
            :class="{ checked: filters.state.birthdayWithin7d }"
            title="KH có sinh nhật trong 7 ngày tới"
            @click="filters.state.birthdayWithin7d = !filters.state.birthdayWithin7d"
          >
            <div class="sfb-row-left"><span class="sfb-row-icon">🎂</span><span class="sfb-row-lbl">Sinh nhật 7 ngày tới</span></div>
            <span v-if="eventCounts.birthday > 0" class="sfb-row-count">{{ eventCounts.birthday }}</span>
          </div>
          <div
            class="sfb-row"
            :class="{ checked: filters.state.appointmentWithin24h }"
            title="Có lịch hẹn trong 24h tới"
            @click="filters.state.appointmentWithin24h = !filters.state.appointmentWithin24h"
          >
            <div class="sfb-row-left"><span class="sfb-row-icon">📞</span><span class="sfb-row-lbl">Lịch hẹn 24h tới</span></div>
            <span v-if="eventCounts.appointmentSoon > 0" class="sfb-row-count amber">{{ eventCounts.appointmentSoon }}</span>
          </div>
          <div
            class="sfb-row"
            :class="{ checked: filters.state.appointmentOverdue }"
            title="Lịch hẹn đã qua hạn chưa xử lý"
            @click="filters.state.appointmentOverdue = !filters.state.appointmentOverdue"
          >
            <div class="sfb-row-left"><span class="sfb-row-icon">⚠️</span><span class="sfb-row-lbl">Hẹn quá hạn</span></div>
            <span v-if="eventCounts.appointmentOverdue > 0" class="sfb-row-count red">{{ eventCounts.appointmentOverdue }}</span>
          </div>
        </div>
      </div>

      <!-- ══ PHỤ TRÁCH DROPDOWN ══ -->
      <div v-if="openDropdown === 'sale'" class="sfb-panel" @click.stop>
        <div class="sfb-panel-header">
          <span class="sfb-panel-title"><UserRoundCogIcon :size="13" :stroke-width="2" /> Phụ trách</span>
          <button class="sfb-panel-close" @click="openDropdown = null"><XIcon :size="14" :stroke-width="2" /></button>
        </div>
        <div class="sfb-panel-body">
          <div class="sfb-row" :class="{ checked: filters.state.saleAssigneeId === null }" @click="filters.state.saleAssigneeId = null">
            <div class="sfb-row-left"><span class="sfb-radio"></span><span class="sfb-row-lbl">Tôi phụ trách</span></div>
          </div>
          <div class="sfb-row" :class="{ checked: filters.state.saleAssigneeId === 'all' }" @click="filters.state.saleAssigneeId = 'all'">
            <div class="sfb-row-left"><span class="sfb-radio"></span><span class="sfb-row-lbl">Tất cả sale</span></div>
          </div>
          <div class="sfb-row" :class="{ checked: filters.state.saleAssigneeId === 'unassigned' }" @click="filters.state.saleAssigneeId = 'unassigned'">
            <div class="sfb-row-left"><span class="sfb-radio"></span><span class="sfb-row-lbl">Chưa giao sale</span></div>
          </div>
        </div>
      </div>

      <!-- Chip bar: các filter đang áp — đã chứa trong hàng button, không cần dòng riêng nữa -->
      <!-- (removed 2026-07-22: chips giờ hiển thị inline ngạy sau các nút dropdown) -->
    </div>
    ...
    <div class="cfb-mini">
      ...
      <div class="mini-right">
        ...
        <button
          class="mini-collapse-btn"
          :class="{ 'is-collapsed': collapsed, 'has-active': hasAnyActiveFilter }"
          @click="$emit('update:collapsed', !collapsed)"
          :title="collapsed ? 'Hiện bộ lọc' : 'Thu gọn bộ lọc'"
        >
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
            <polyline points="18 15 12 9 6 15" />
          </svg>
          <span class="collapse-dot" v-if="collapsed && hasAnyActiveFilter"></span>
        </button>
      </div>
    </div>
  </div>
