  ...
  <v-dialog :model-value="modelValue" max-width="560" @update:model-value="$emit('update:modelValue', $event)">
    <v-card>
      ...
      <v-card-text>
        ...
        />
        <div class="aic-provider-actions mb-3">
          <button class="aic-link" :disabled="loadingProviders || addingCustom" @click="addCustom">
            <v-icon size="15">mdi-plus</v-icon> Thêm custom provider
          </button>
          <template v-if="currentProvider?.isCustom">
            <button class="aic-link" :disabled="savingProvider" @click="renameCustom">
              <v-icon size="15">mdi-pencil-outline</v-icon> Đổi tên
            </button>
            <button class="aic-link aic-link-danger" :disabled="savingProvider" @click="deleteCustom">
              <v-icon size="15">mdi-trash-can-outline</v-icon> Xoá provider
            </button>
          </template>
        </div>
        ...
        <v-text-field
          ...
          label="Base URL"
          :placeholder="currentProvider?.isCustom ? 'https://openrouter.ai/api  ·  http://localhost:11434' : 'https://api.openai.com'"
          class="mb-1"
          density="comfortable"
          :rules="baseUrlRules"
        />
        <div v-if="currentProvider?.isCustom" class="text-caption text-medium-emphasis mb-3">
          Nhập URL gốc, có hoặc không kèm <code>/v1</code> đều được (vd <code>https://ramclouds.me/v1</code>). Hệ thống tự chuẩn hoá rồi gọi <code>/v1/chat/completions</code> và <code>/v1/models</code> theo chuẩn OpenAI.
        </div>
...
type ProviderInfo = { id: string; name: string; baseUrl: string; hasKey: boolean; keyMask: string; isCustom: boolean };
...
const providerItems = computed(() => providers.value.map((p) => ({ title: p.isCustom ? `${p.name} (custom)` : p.name, value: p.id })));
const currentProvider = computed(() => providers.value.find((p) => p.id === local.provider));
const baseUrlRules = computed(() =>
  currentProvider.value?.isCustom ? [(v: string) => !!(v && v.trim()) || 'Custom provider bắt buộc nhập Base URL'] : [],
);
...
async function fetchModels() {
  ...
  modelsError.value = '';
  try {
    const res = await api.get(`/ai/providers/${encodeURIComponent(local.provider)}/models`);
...
async function putProvider(payload: { apiKey?: string | null; baseUrl?: string | null }) {
  ...
  try {
    await api.put(`/ai/providers/${encodeURIComponent(local.provider)}`, payload);
...
/* Thêm custom provider mới (OpenAI-compatible) */
async function addCustom() {
  const name = window.prompt('Tên hiển thị cho custom provider (vd: OpenRouter, DeepSeek, Ollama):')?.trim();
  if (!name) return;
  addingCustom.value = true;
  try {
    const res = await api.post('/ai/providers/custom', { name });
    await fetchProviders();
    if (res.data?.id) {
      local.provider = res.data.id;
      onProviderChange();
    }
  } catch (e: any) {
    window.alert(e?.response?.data?.error || 'Thêm custom provider thất bại');
  } finally {
    addingCustom.value = false;
  }
}

/* Đổi tên custom provider hiện tại */
async function renameCustom() {
  if (!currentProvider.value?.isCustom) return;
  const name = window.prompt('Tên mới:', currentProvider.value.name)?.trim();
  if (!name || name === currentProvider.value.name) return;
  savingProvider.value = true;
  try {
    await api.patch(`/ai/providers/custom/${encodeURIComponent(local.provider)}`, { name });
    await fetchProviders();
  } catch (e: any) {
    window.alert(e?.response?.data?.error || 'Đổi tên thất bại');
  } finally {
    savingProvider.value = false;
  }
}

/* Xoá custom provider hiện tại */
async function deleteCustom() {
  if (!currentProvider.value?.isCustom) return;
  if (!window.confirm(`Xoá provider "${currentProvider.value.name}"? Key + Base URL của nó sẽ bị xoá.`)) return;
  savingProvider.value = true;
  try {
    await api.delete(`/ai/providers/custom/${encodeURIComponent(local.provider)}`);
    await fetchProviders();
    local.provider = providers.value[0]?.id ?? 'openai';
    onProviderChange();
  } catch (e: any) {
    window.alert(e?.response?.data?.error || 'Xoá thất bại');
  } finally {
    savingProvider.value = false;
  }
}
...
/* Hàng nút thao tác provider (thêm/đổi tên/xoá custom) */
.aic-provider-actions { display: flex; flex-wrap: wrap; gap: 12px; }
.aic-link {
  display: inline-flex; align-items: center; gap: 3px;
  background: none; border: none; padding: 0; cursor: pointer;
  font-size: 12.5px; font-weight: 600; color: #1786be;
}
.aic-link:hover:not(:disabled) { text-decoration: underline; }
.aic-link-danger { color: #B91C1C; }
.aic-link:disabled { opacity: .4; cursor: not-allowed; }
</style>
