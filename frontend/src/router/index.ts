  ...
  // ════════ NEW Settings — 6-group sidebar layout ════════
  {
    ...
    children: [
      ...
      },
      {
        ...
        component: () => import("@/views/settings/SystemNotificationsPage.vue"),
        meta: { resource: "settings" },
      },
      {
        path: "org/billing",
      ...
      },
      {
        ...
        component: () => import("@/views/rbac/PermissionGroupsView.vue"),
        meta: { resource: "permission_group" },
      },
      {
        path: "rbac/users",
        name: "Settings.RbacUsers",
        component: () => import("@/views/rbac/UsersRbacView.vue"),
        meta: { resource: "user" },
      },
      {
        path: "rbac/network",
        name: "Settings.RbacNetwork",
      ...
      {
        path: "crm/tags-v2",
        name: "Settings.TagsV2",
        component: () => import("@/views/settings/TagTaxonomyV2Page.vue"),
        meta: { resource: "settings" },
      },
      {
        path: "crm/zalo-labels",
      ...
      // 2026-06-18 — Trần SDK dời sang Cài đặt (gate 'settings', KHÔNG 'zalo_account') → sale ko đổi được.
      {
        ...
        component: () => import("@/views/settings/SdkLimitsSettingsPage.vue"),
        meta: { resource: "settings" },
      },
      // Facebook Lead Ads route → extension bundle (eeSettingsChildren).
      {
        path: "channels/rate-limit",
        name: "Settings.RateLimit",
        component: () => import("@/views/settings/SettingsComingSoon.vue"),
        props: { feature: "rate-limit" },
        meta: { resource: "settings" },
      },
      ...
      {
        path: "channels/hisweetie-pos",
        name: "Settings.HisweetiePos",
        component: () => import("@/views/HisweetiePosView.vue"),
        meta: { resource: "settings" },
      },
      ...
      // 🛠 Dev & API
      {
        ...
        meta: { resource: "webhook" },
      },
      {
        path: "dev/public-token",
        name: "Settings.PublicToken",
        component: () => import("@/views/settings/SettingsComingSoon.vue"),
        props: { feature: "public-token" },
        meta: { resource: "settings" },
      },
      {
        path: "dev/feature-flags",
        name: "Settings.FeatureFlags",
        component: () => import("@/views/settings/SettingsComingSoon.vue"),
        props: { feature: "feature-flags" },
        meta: { resource: "settings" },
      },
      {
        path: "dev/backup",
    ...
    ? [
        {
          path: "/marketing",
          component: () =>
            import("@/views/marketing/CommunityMarketingShell.vue"),
          meta: { requiresAuth: true },
          children: [
            ...
            // Tệp khách hàng (Customer Lists) — open-core, dùng được ở Community.
            {
              path: "lists",
              name: "CE.Lists",
              component: () => import("@/views/marketing/ListsView.vue"),
              meta: { requiresAuth: true },
            },
            {
              path: "lists/:id",
              name: "CE.ListDetail",
              component: () => import("@/views/marketing/ListDetailView.vue"),
              meta: { requiresAuth: true },
            },
          ],
        } as RouteRecordRaw,
...
router.beforeEach(async (to, _from, next) => {
  ...
  if (to.meta.requiresAuth) {
    ...
    if (
      required &&
      !managerBypass &&
      !authStore.canAccess(required, (to.meta.action as string) ?? "access")
    ) {
      try {
...
// brand. Title dạng "Tên màn hình · Hi-CRM".
const BRAND = "Hi-CRM";
