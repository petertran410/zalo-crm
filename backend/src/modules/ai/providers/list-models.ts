...
export async function listProviderModels(
  ...
  switch (provider) {
    ...
    case 'gemini':
      ...
      break;
    default:
      /* Custom provider (custom:<slug>) — OpenAI-compatible /v1/models. Strip /v1 cuối
         (nếu user nhập sẵn) để tránh /v1/v1. Endpoint nào không hỗ trợ → ném lỗi,
         route trả {models:[],error} cho UI fallback gõ tay. */
      if (provider.startsWith('custom:')) {
        const root = baseUrl.replace(/\/+$/, '').replace(/\/v1$/i, '');
        models = await listOpenaiCompat(root, apiKey, '/v1/models');
        break;
      }
