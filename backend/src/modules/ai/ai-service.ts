...
import { getProviderConfig, getAvailableProviders, resolveProviderApiKey, getProviderBaseUrl, isCustomProvider, isSelectableProvider } from './provider-registry.js';
...
export async function updateAiConfig(orgId: string, input: { provider?: string; model?: string; maxDaily?: number; enabled?: boolean }) {
  // Guard: nếu đổi provider sang custom id, id đó phải đã đăng ký (tránh lưu provider rác).
  if (input.provider !== undefined && isCustomProvider(input.provider)) {
    const ok = await isSelectableProvider(orgId, input.provider);
    if (!ok) throw new Error('Custom provider không tồn tại');
  }
...
export async function generateText(provider: string, apiKey: string, model: string, system: string, prompt: string, maxTokens?: number, baseUrlOverride?: string) {
  ...
  /* Custom provider (custom:<slug>) — OpenAI-compatible. baseUrl gốc, nối /v1/chat/completions.
     Strip /v1 cuối (nếu user đã nhập sẵn) để tránh /v1/v1. */
  if (isCustomProvider(provider)) {
    const root = baseUrl.replace(/\/+$/, '').replace(/\/v1$/i, '');
    return generateWithOpenaiCompat(`${root}/v1/chat/completions`, apiKey, model, system, prompt, maxTokens);
  }
