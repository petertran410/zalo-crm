...
export async function aiRoutes(app: FastifyInstance) {
  ...
  /* Set/xoá API key + base URL của 1 provider (per-org). apiKey rỗng = xoá → fallback .env.
     :id có thể là builtin (openai…) hoặc custom (custom%3Aslug — URL-encoded). */
  ...
  /* ── Custom provider CRUD (OpenAI-compatible, nhiều cái, tên tùy biến) ───────── */

  /* Thêm custom provider mới → trả {id,name}. */
  app.post('/api/v1/ai/providers/custom', { preHandler: requireGrant('settings', 'edit') }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const body = (request.body ?? {}) as { name?: string };
      const created = await addCustomProvider(request.user!.orgId, body.name ?? '');
      return created;
    } catch (err) {
      logger.error('[ai] Add custom provider error:', err);
      return reply.status(400).send({ error: (err as Error).message || 'Failed to add custom provider' });
    }
  });

  /* Đổi tên custom provider. :id = custom%3Aslug (URL-encoded). */
  app.patch('/api/v1/ai/providers/custom/:id', { preHandler: requireGrant('settings', 'edit') }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const id = decodeURIComponent((request.params as { id: string }).id);
      if (!isCustomProvider(id)) return reply.status(400).send({ error: 'Not a custom provider' });
      const body = (request.body ?? {}) as { name?: string };
      await renameCustomProvider(request.user!.orgId, id, body.name ?? '');
      return { ok: true };
    } catch (err) {
      logger.error('[ai] Rename custom provider error:', err);
      return reply.status(400).send({ error: (err as Error).message || 'Failed to rename custom provider' });
    }
  });

  /* Xoá custom provider + key/baseUrl. Nếu AiConfig đang trỏ vào id này → reset về default. */
  app.delete('/api/v1/ai/providers/custom/:id', { preHandler: requireGrant('settings', 'edit') }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const orgId = request.user!.orgId;
      const id = decodeURIComponent((request.params as { id: string }).id);
      if (!isCustomProvider(id)) return reply.status(400).send({ error: 'Not a custom provider' });
      await removeCustomProvider(orgId, id);
      invalidateModelCache(orgId, id);
      // Reset AiConfig nếu đang dùng provider vừa xoá (tránh provider mồ côi).
      const cfg = await prisma.aiConfig.findUnique({ where: { orgId }, select: { provider: true } });
      if (cfg?.provider === id) {
      ...
      logger.error('[ai] Delete custom provider error:', err);
      return reply.status(400).send({ error: (err as Error).message || 'Failed to delete custom provider' });
    }
  });
