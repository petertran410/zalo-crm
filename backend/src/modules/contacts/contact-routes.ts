...
export async function contactRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/contacts', { preHandler: requireGrant('contact', 'access') }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (relationshipKindAny) {
        ...
        if (kinds.length > 0) {
          where.AND = where.AND ?? [];
          where.AND.push({ friends: { some: { relationshipKind: { in: kinds } } } });
        }
      }
  ...
  app.get('/api/v1/contacts/stats', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const cScope = await getContactScope(user.id, user.orgId, user.role);
      const base: any = { orgId: user.orgId, mergedInto: null, archivedAt: null };
      if (!cScope.isOrgAdmin && cScope.accessibleContactIds !== null) {
  ...
  app.get('/api/v1/contacts/sources', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const grouped = await prisma.contact.groupBy({
        by: ['source'],
        where: { orgId: user.orgId, mergedInto: null, archivedAt: null, source: { not: null } },
        _count: { source: true },
  ...
  app.get('/api/v1/contacts/pipeline', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const cScope = await getContactScope(user.id, user.orgId, user.role);
      const scopeWhere: any = { orgId, status: { not: null }, mergedInto: null, archivedAt: null };
      if (!cScope.isOrgAdmin && cScope.accessibleContactIds !== null) {
      ...
      await Promise.all(
        statuses.map(async (st) => {
          const where: any = { orgId, status: st ?? null, mergedInto: null, archivedAt: null };
          if (!cScope.isOrgAdmin && cScope.accessibleContactIds !== null) {
            where.id = { in: cScope.accessibleContactIds };
          }
  ...
  app.post('/api/v1/contacts/bulk-archive', {
    ...
    try {
      ...
      return reply.status(500).send({ error: 'Failed to bulk archive contacts' });
    }
  });
  ...
  app.post('/api/v1/contacts/bulk-restore', {
    ...
    try {
      ...
      const result = await prisma.contact.updateMany({
        where: { id: { in: ids }, orgId: user.orgId, archivedAt: { not: null } },
        data: { archivedAt: null, archivedById: null },
      });
      return { success: true, restored: result.count };
    } catch (err) {
      logger.error('[contacts] Bulk restore error:', err);
      return reply.status(500).send({ error: 'Failed to bulk restore contacts' });
    }
  });
  ...
  app.delete('/api/v1/contacts/:id/permanent', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.status(500).send({ error: 'Failed to permanently delete contact' });
    }
  });
  ...
  app.post('/api/v1/contacts/bulk-purge', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.status(500).send({ error: 'Failed to bulk purge contacts' });
    }
  });
